...
func (in *IstioConfigService) CreateIstioConfigDetail(cluster, namespace, resourceType string, body []byte) (models.IstioConfigDetails, error) {
	...
	//can i use kube? raelly?
	case kubernetes.DeploymentType:
		istioConfigDetail.Deployment = &apps_v1.Deployment{}
		err = json.Unmarshal(body, istioConfigDetail.Deployment)
		if err != nil {
			return istioConfigDetail, api_errors.NewBadRequest(err.Error())
		}
		istioConfigDetail.Deployment, err = in.userClients[cluster].Kube().AppsV1().Deployments(namespace).Create(ctx, istioConfigDetail.Deployment, createOpts)
